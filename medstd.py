...
README_TEXT = {
    ...
    "outgoing": (
        "# 08-Outgoing\n\n"
        "Subfolders here are **recipients**, mirroring `02-Incoming`. Inside each\n"
        "recipient's folder, one frozen folder per **issue event**, date-prefixed:\n\n"
        "    02-Architect/\n"
        "        260910-S3-Design_team_review/\n"
        "        261104-A1-Building_control_pack/\n"
        "    05-Contractor/\n"
        "        261120-S4-Tender_issue/\n\n"
        "Format for the event folder is `YYMMDD-<status>-<purpose>`.\n\n"
        "**This structure is the issue record** - it is the practice's substitute\n"
        "for a CDE audit trail, so nothing in a dated event folder is ever edited\n"
        "after the issue goes out. To correct an issue, make a new one.\n\n"
        ...
        "If an issue goes to more than one recipient at once, pick the primary\n"
        "recipient's folder and note the rest in `REGISTER.csv` - don't duplicate\n"
        "the files. Log every issue there: one row per file per issue.\n"
    ),
    "superseded": (
        "# XX-Superseded\n\n"
